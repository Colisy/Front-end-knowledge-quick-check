...
- 2xx - **成功系列**
  ...
  204  - 返回响应头，不带响应体(如：收集访问者的统计信息，打点请求)
  
  206  - 返回部分数据(如：下载工具，断点续传)
...
- 3xx - **重定向系列**
  ...
  **304 - 没有更改(主要用来处理协商缓存)**
  ...
  307 - 不改变请求方法的临时重定向(302可能会将请求方式转为get)

- 4xx - **客户端错误系列**⚠️

  400 - 请求报文语法错误(服务器得到响应但没有可以返回的数据)
...
- 服务器产生，浏览器存储，每次http请求会自动携带cookie,跨域不会自动携带
- 属性
...
4. 缓存策略
   ...
   - http缓存
5. cookie 是紧跟域名的。同一个域名下的所有请求，都会携带 Cookie。
